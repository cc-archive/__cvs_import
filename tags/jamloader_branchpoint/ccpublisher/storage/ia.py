...
from storage.basic import MetaElement
from interface import TEXT, COMBO
from license import LicenseMeta

class WorkMeta(storage.basic.MetadataSection):
    ...
    def __init__(self):
        storage.basic.MetadataSection.__init__(self,'workmeta', 'Tell Us About Your File', self.FIELDS )
    
class ArchiveMeta(storage.basic.MetadataSection):
    zope.interface.implements(interface.ISection)
    FIELDS = [storage.basic.MetaElement(interface.TEXT,
                    'keywords', '', '', 'Keywords')]
    def __init__(self):
        storage.basic.MetadataSection.__init__(self,'ameta', 'Tell Us More About Your File', self.FIELDS )
    
class ArchiveDetails(storage.basic.MetadataSection):
    zope.interface.implements(interface.ISection)
    visible = False

    FIELDS = [storage.basic.MetaElement(interface.TEXT,
                    'iausername'),
              storage.basic.MetaElement(interface.TEXT,
                    'iapasswd'),]
                    
    def __init__(self):
        storage.basic.MetadataSection.__init__(self, 'adetails', '', self.FIELDS )
...
class ArchiveStorage:
    ...
    def addFile(self, filename, metadata):
        """Adds a file to the current submission; filename is a string,
        metadata is an instance of StorageFileMetadata."""
        self.fileList.append( (filename, metadata) )
