...
CREATE TABLE content_raw (
	...
	license                 VARCHAR(250), -- This will contain the appropriate Creative Commons license
        num_downloads		INTEGER,
	-- To find out a song's current rating, total_ratings / num_ratings
	num_ratings		INTEGER,
	total_ratings		INTEGER,
        idxFTI                  to_tsvector('default',coalesce(songname,'')||' '||coalesce(filename,''))
);
...
CREATE TABLE content_versions (
	...
	description             VARCHAR(300),
        idxFTI                  to_tsvector('default',coalesce(one_line_summary,'')||' '||coalesce(body,'')||' '||coalesce(description,''))
);
