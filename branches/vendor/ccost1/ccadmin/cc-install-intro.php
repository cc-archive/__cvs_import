<h2>Welcome to CC Host Installation</h2>
...
<p>In order to run CC Host you need to install some other software and perform
one database task.</p>
<ul>
<li><b>PHP 4</b>
<p> <? print $vmsg; ?></p></li>
...
<li><b>mySQL</b>
<p>Currently mySQL is the only database supported by CC Host. No doubt this will change someday but for now
this is the only one. While there is no known reason why it shouldn't work on any moderately recent version
it should be noted the CC Host code has only been tested on a few version <b>4</b> installations.</p></li>

<li><b>Create a Database</b>
<p>Before you continue installing CC Host you need to create a database. (If you are running at a hosted site
...
<p>CC Host uses the <a href="http://www.getid3.org/">GetID3 library</a> to verify 
...
might have on the same server as CC Host. If you want to give CC Host users the ability to comment
on uploads, bascially creating threads for each one, CC Host knows how to tightly integrate into
...
number when you are ready to install CC Host.</p>
