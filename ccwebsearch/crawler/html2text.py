...
re_script = re.compile(r'<script.*?>.*?</script>', re.I|re.M|re.S)
re_style = re.compile(r'<style.*?>.*?</style>', re.I|re.M|re.S)
re_funny = re.compile(r'[^\s\w:/.,?!-@$]', re.M|re.S)
...
class _html2text:
	def __call__(self, html, basehref, maxlen=80, showtags=0, showlinks=1, simulateem=0):
		self.text, self.line, self.maxlen, self.simulateem  = '', '', maxlen, simulateem
		...
		if showlinks:
			...
			for u in href_urls:
				...
				if self.simulateem:
					self.text += "\n[" + `i` + "]"
				self.text += (' ' * (len(`len(href_urls)`) - len(`i`) + 1)) + \
...
if __name__ == "__main__":
	...
	o = html2text(urllib.urlopen(url).read(), url, maxlen=maxlen)
	try:
